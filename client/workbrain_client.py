...
def process_media_file(file_path, media_type='image', max_size=1024, quality=85):
    ...
    try:
        ...
        if media_type == 'image':
            ...
            print(f"文档大小: {processed_size} 字节")
            
            # --- 生成Web URL --- 
            filename = os.path.basename(file_path) # 获取包含UUID前缀的文件名
            web_url = f"/static/uploads/{filename}" 
            print(f"文档Web URL: {web_url}")
            ...
            try:
                if file_ext == 'txt':
                    with open(file_path, 'r', encoding='utf-8', errors='ignore') as text_file:
                        ...
                        print(f"从文本文件中提取了 {len(extracted_text)} 个字符")
                elif file_ext == 'pdf':
                    try:
                        import PyPDF2
                        with open(file_path, 'rb') as pdf_file:
                            ...
                            print(f"从PDF文件中提取了 {len(extracted_text)} 个字符")
                    except ImportError:
                        print("未安装PyPDF2库，无法提取PDF文本")
                    ...
                    try:
                        if file_ext == 'docx':
                            ...
                            print(f"从Word文档中提取了 {len(extracted_text)} 个字符")
                    except ImportError:
                        print("未安装python-docx库，无法提取Word文本")
                elif file_ext in ['xls', 'xlsx']:
                    try:
                        import pandas as pd
                        df = pd.read_excel(file_path)
                        extracted_text = df.to_string(index=False)
                        print(f"从Excel文件中提取了 {len(extracted_text)} 个字符")
                    except ImportError:
                        print("未安装pandas或openpyxl库，无法提取Excel文本")
            except Exception as e:
                print(f"提取文档文本时出错: {str(e)}")
                traceback.print_exc()
                
            print(f"文档处理完成: 格式={file_ext}")
            # 返回实际的Web URL，大小和提取的文本
            return web_url, processed_size, extracted_text
        ...
        # 对于文档类型，需要返回三个值(data_uri, size, extracted_text)
...
class MCPWebClient:
    ...
    def __init__(self):
        ...
        if not system_message:
            print("从环境变量中未找到SYSTEM_PROMPT，使用默认系统提示词")
            # 使用默认的系统提示词
            ...
            print(f"使用环境变量中的系统提示词: {system_message[:50]}...")
    ...
    async def _process_tool_result(self, result, tool_name, tool_call_id):
        ...
        try:
            ...
            # 尝试解析JSON结果，特别是处理web_research的输出
            try:
                parsed_result = json.loads(result_content)
                if isinstance(parsed_result, dict) and parsed_result.get("type") == "search_results":
                    # 这是web_research的结构化结果
                    print("处理web_research的结构化搜索结果")
                    message_data = {
                        'type': 'assistant', # 显示为助手消息
                        'content': parsed_result.get('summary_text', '搜索完成。'), # 使用JSON中的摘要文本
                        'media_type': 'search_result', # 特殊类型，指示前端渲染搜索卡片
                        'media_content': parsed_result, # 传递完整的JSON数据给前端
                        'is_tool_result': True,
                        'tool_name': tool_name
                    }
                    ...
                    # 处理来自工具的错误JSON
                    print(f"工具 {tool_name} 返回错误: {parsed_result.get('message')}")
                    content = f"工具 {tool_name} 执行出错: {parsed_result.get('message')}"
                    media_type = 'text'
                else:
                    # 不是期望的JSON格式，按旧逻辑处理
            ...
            # 针对multimodal_content分析工具的特殊处理（移到JSON解析之后）
            if tool_name == "analyze_multimodal_content":
            ...
            if media_type != 'text':
                try:
                    # 对于媒体结果，尝试解析JSON获取文件名
                    json_data = json.loads(result_content)
                    if isinstance(json_data, dict):
                        # 首先检查是否有description字段包含文件名信息
                        if 'description' in json_data:
                            desc = json_data.get('description', '')
                            # 从描述中提取文件名
                            if "生成的" in desc and "'" in desc:
                                parts = desc.split("'")
                                if len(parts) > 2:
                                    # 提取引号内的内容作为文件名
                                    filename = parts[1]
                                    if not filename.endswith(('.jpg', '.png', '.mp3', '.mp4')):
                                        # 根据媒体类型添加适当后缀
                                        ext = '.jpg' if media_type == 'image' else '.mp4' if media_type == 'video' else '.mp3'
                                        filename += ext
                        
                        # 如果URL字段存在并且是文件路径
                        if not filename and 'url' in json_data and '/' in json_data['url']:
                            # 从URL中提取文件名
                            url_filename = json_data['url'].split('/')[-1]
                            # 检查是否有UUID前缀，如果有则移除
                            if '_' in url_filename:
                                # 确保不会意外删除文件扩展名中的下划线
                                parts = url_filename.split('_', 1)
                                url_filename = parts[1] if len(parts) > 1 else url_filename
                            filename = url_filename
                except:
                    pass
                
                # 如果没有找到文件名，生成一个基于工具名称的默认名称
                if not filename:
                    base_name = tool_name.replace("generate_", "")
                    ext = '.jpg' if media_type == 'image' else '.mp4' if media_type == 'video' else '.mp3'
            ...
            # 如果有文件名，添加到消息数据中
            if filename and media_type != 'text':
                message_data['filename'] = filename
                print(f"添加文件名到工具结果: {filename}")
        ...
        try:
            ...
            if media_files and media_files.get('has_media'):
                ...
                for file_info in media_files.get('files', []):
                    ...
                    print(f"处理{file_type}文件: {file_path}, 原始文件名: {original_filename}")
                    ...
                    try:
                        ...
                        if file_type == 'image':
                            ...
                            user_content.append({
                                ...
                                'filename': original_filename  # 添加原始文件名
                            })
                            ...
                            print(f"检测到文档: {file_path}, 文件名: {original_filename}")
                            # 处理文档
                            data_uri, _, extracted_text = process_media_file(file_path, media_type='document')
                            ...
                            # 将提取的文本添加到用户输入
                            if extracted_text:
                                print(f"从文档中提取了文本，长度为 {len(extracted_text)} 字符")
                                # 提取文本过长时进行摘要
                                if len(extracted_text) > 3000:
                                    print("文档文本过长，进行截断")
                                    extracted_text = extracted_text[:3000] + "...(文档内容过长，已截断)"
            ...
            if document_text:
                if enhanced_query:
                    ...
                    enhanced_query = f"请分析这个文档：\n\n{document_text}"
                print(f"整合了文档文本到查询中，现在查询长度为 {len(enhanced_query)} 字符")
    ...
    def _send_user_message_to_frontend(self, query, media_files):
        ...
        try:
                ...
                for file_info in media_files.get('files', []):
                    ...
                    try:
                        ...
                        if file_type == 'image':
                            ...
                            message_data['filename'] = original_filename  # 添加原始文件名
                            print(f"成功处理图片文件: {original_filename}")
                            ...
                            message_data['media_content'] = data_uri
                            message_data['filename'] = original_filename  # 添加原始文件名
                            print(f"成功处理视频文件: {original_filename}")
                            break  # 前端目前只支持每条消息显示一个媒体文件
                            ...
                            message_data['media_content'] = data_uri
                            message_data['filename'] = original_filename  # 添加原始文件名
                            print(f"成功处理音频文件: {original_filename}")
                            break  # 前端目前只支持每条消息显示一个媒体文件
                            ...
                            message_data['media_content'] = data_uri
                            message_data['filename'] = original_filename  # 添加原始文件名
                            print(f"成功处理文档文件: {original_filename}")
                            # 不将提取的文本添加到前端显示的消息中，仅在后端处理
                            break  # 前端目前只支持每条消息显示一个媒体文件
            ...
            if message_data['content'].strip() or 'media_type' in message_data:
                print(f"发送用户消息到前端: {message_data.get('filename', '无文件名')}")
                socketio.emit('message', message_data)
            else:
        ...
        try:
            ...
            for chunk in response:
                ...
                if (hasattr(chunk, 'choices') and chunk.choices and 
                    ...
                    # 过滤掉网络搜索相关的JSON内容
                    if '```search_results' not in buffer and '```search_results' in (buffer + chunk_content):
                        # 如果即将添加的内容包含搜索结果开始标记
                        buffer_parts = (buffer + chunk_content).split('```search_results')
                        buffer = buffer_parts[0]  # 保留搜索结果前的内容
                        print("检测到搜索结果开始标记，将被过滤")
                        continue
                    elif '```search_results' in buffer:
                        # 已经在搜索结果块内
                        if '```' in chunk_content:
                            # 找到结束标记，恢复正常添加
                            buffer_parts = chunk_content.split('```', 1)
                            if len(buffer_parts) > 1:
                                chunk_content = buffer_parts[1]  # 只保留搜索结果后的内容
                                print("检测到搜索结果结束标记，恢复正常输出")
                            else:
                                continue  # 跳过这个块
                        else:
                            continue  # 跳过搜索结果块内的内容
                    
                    buffer += chunk_content
                    socketio.emit('message', {'content': chunk_content, 'type': 'assistant'})
    ...
    async def _generate_followup_response(self):
        """生成模型的后续回应"""
        try:
            # --- START MODIFICATION: Find recent media result (Corrected Logic) ---
            last_media_result = None
            media_generation_tool_names = ['generate_image', 'generate_speech_stream', 'generate_video']
            
            # 用于存储已遇到的媒体生成工具调用信息 {tool_call_id: tool_name}
            found_media_assistant_calls = {}
            ...
            for i in range(len(self.messages) - 1, -1, -1):
                ...
                if msg.get('role') == 'tool':
                    ...
                    # 检查这个结果是否对应我们已找到的媒体生成工具调用
                    if tool_call_id and tool_call_id in found_media_assistant_calls:
                        tool_name = found_media_assistant_calls[tool_call_id]
                        print(f"找到对应的工具结果，ID: {tool_call_id}, 工具名: {tool_name}")
                        try:
                            # 确保 content 是字符串
                            ...
                            # 检查是否是成功的媒体结果 (type不是error, 且有url)
                            if isinstance(result_json, dict) and result_json.get('type') != 'error' and result_json.get('url'):
                                media_type = result_json.get('type')
                                media_url = result_json.get('url')
                                ...
                                if 'description' in result_json:
                                    ...
                                    if "生成的" in desc and "'" in desc:
                                        ...
                                        if len(parts) > 2:
                                            filename = parts[1]
                                            if not filename.lower().endswith(('.jpg', '.png', '.mp3', '.mp4', '.wav')):
                                                ext_map = {'image': '.jpg', 'video': '.mp4', 'audio': '.mp3'}
                                                filename += ext_map.get(media_type, '')
                                # 2. URL
                                if not filename and '/' in media_url:
                                    url_filename = media_url.split('/')[-1]
                                    if '_' in url_filename and len(url_filename.split('_', 1)) > 1:
                                         url_filename = url_filename.split('_', 1)[1]
                                    filename = url_filename
                                # 3. Default
                                if not filename:
                                    base_name = tool_name.replace("generate_", "")
                                    ext_map = {'image': '.jpg', 'video': '.mp4', 'audio': '.mp3'}
                                    filename = f"{base_name}_{int(time.time())}{ext_map.get(media_type, '')}"
                                ...
                                print(f"解析到最新的媒体结果: 类型={media_type}, URL={media_url}, 文件名={filename}")
                                break # 找到最新的一个即可，停止遍历
                            else:
                                print(f"工具结果不是有效的成功媒体JSON: {tool_result_content[:100]}...")
                        except json.JSONDecodeError:
                            print(f"工具结果不是有效的JSON: {tool_result_content[:100]}...")
                        except Exception as parse_error:
                            print(f"解析工具结果时出错: {parse_error}")
                            traceback.print_exc()
                        
                        # 找到对应的tool结果（无论成功与否）后，不再需要检查更早的assistant消息中是否调用了此工具
                        # 但我们仍然需要继续向后遍历，因为可能有更早的、成功的媒体生成
                        # 从字典中移除已处理的tool_call_id
                        del found_media_assistant_calls[tool_call_id]
                        # 如果已经找到有效的last_media_result，则可以停止
                        if last_media_result:
                            break
                        else:
                            continue # 继续向后查找其他可能的媒体结果

                # 如果是助手消息，记录其中的媒体生成工具调用
                elif msg.get('role') == 'assistant' and msg.get('tool_calls'):
                    for tool_call in msg['tool_calls']:
                        tool_name = tool_call.get('function', {}).get('name')
                        tool_call_id = tool_call.get('id')
                        if tool_name in media_generation_tool_names and tool_call_id:
                             # 只有当这个tool_call_id还没被对应的tool结果处理时才添加
                            if tool_call_id not in [m.get('tool_call_id') for m in self.messages[i+1:] if m.get('role') == 'tool']:
                                found_media_assistant_calls[tool_call_id] = tool_name
                                print(f"记录媒体生成工具调用: {tool_name}, ID: {tool_call_id}")
                
                # 如果已经找到了有效的last_media_result，就没必要再往前找了
                if last_media_result:
                    break
            ...
            if last_media_result:
                 ...
                 print("在生成最终回应前，未找到有效的、最近的媒体生成结果可供附加")
            # --- END MODIFICATION ---
            ...
            # 处理流式响应
            buffer = ""
            # --- START MODIFICATION: Flag for first chunk ---
            is_first_chunk = True 
            # --- END MODIFICATION ---
            for chunk in response:
                if (hasattr(chunk, 'choices') and chunk.choices and 
                    ...
                    # --- START MODIFICATION: Emit first chunk with media ---
                    message_to_emit = {'content': chunk_content, 'type': 'assistant'}
                    if is_first_chunk and last_media_result:
                        message_to_emit.update(last_media_result)
                        print(f"附加媒体信息到第一个消息块: {last_media_result}")
                        is_first_chunk = False # 只附加一次
                        
                    socketio.emit('message', message_to_emit)
                    # --- END MODIFICATION ---
...
def upload_file():
    ...
    try:
        ...
        # 添加对文档类型的支持
            ...
            file_type = 'document'
            print(f"检测到Word文档: {content_type}")
            ...
            file_type = 'document'
            print(f"检测到Excel表格: {content_type}")
            ...
            file_type = 'document'
            print(f"检测到PowerPoint演示文稿: {content_type}")
        elif content_type == 'application/pdf':
            file_type = 'document'
            print(f"检测到PDF文档: {content_type}")
        elif content_type == 'text/plain':
            file_type = 'document'
            print(f"检测到文本文件: {content_type}")
        else:
            file_type = 'other'
        ...
        # 生成唯一文件名
        unique_prefix = str(uuid.uuid4()) + str(int(time.time() * 1000))
        # 保留原始文件名，用于前端显示
...
def run_flask():
    """运行Flask应用"""
    try:
        socketio.run(app, host='0.0.0.0', port=30008, debug=True, use_reloader=True)
    except Exception as e:
